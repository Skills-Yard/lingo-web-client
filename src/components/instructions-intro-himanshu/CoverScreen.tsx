...
// heading column never outgrows the scrollable body. `20rem` is what's left
// once the header (~68px), footer (~76px), body padding and the heading block
// below Robu are subtracted; `clamp`'s 8rem floor keeps him visible on very
// short (landscape phone) viewports, where scrolling is unavoidable anyway.
// `100vh` (not `svh`) to match `main`'s own `h-screen`.
const COVER_ROBU_SIZE =
  "h-[clamp(8rem,calc(100vh_-_20rem),11rem)] w-[clamp(8rem,calc(100vh_-_20rem),11rem)] sm:h-[clamp(8rem,calc(100vh_-_20rem),17.5rem)] sm:w-[clamp(8rem,calc(100vh_-_20rem),17.5rem)] md:h-[clamp(8rem,calc(100vh_-_20rem),22rem)] md:w-[clamp(8rem,calc(100vh_-_20rem),22rem)]";
...
// header + footer + body padding (~156px) plus the intro bubble, the thinking
// illustration and — once revealed — the reveal card. Every tier is scoped
// with `max-[1179px]:` / `min-[1180px]:` (same split point as the rest of this
// screen) rather than plain `sm:`/`md:` + a `min-[1180px]:` override, since
// Tailwind emits the arbitrary breakpoint *before* `md:` and the override
// would lose the cascade (the other flow's CoverScreen hit exactly that).
// Pre-reveal: Robu in the top slot, above the illustration.
const REVEAL_ROBU_SIZE =
  "max-[1179px]:size-[clamp(8rem,calc(100vh_-_23.5rem),11rem)] max-[1179px]:sm:size-[clamp(8rem,calc(100vh_-_27rem),17.5rem)] max-[1179px]:md:size-[clamp(8rem,calc(100vh_-_28.5rem),22rem)] min-[1180px]:size-[clamp(8rem,calc(100vh_-_16rem),22rem)]";
// Revealed: Robu crouched beside the reveal card, which is also on screen.
const REVEALED_ROBU_SIZE =
  "max-[1179px]:size-[clamp(6rem,calc(100vh_-_32.5rem),9rem)] max-[1179px]:sm:size-[clamp(6rem,calc(100vh_-_37.5rem),13.5rem)] max-[1179px]:md:size-[clamp(6rem,calc(100vh_-_39.5rem),17rem)] min-[1180px]:size-[clamp(6rem,calc(100vh_-_28.5rem),18rem)]";
