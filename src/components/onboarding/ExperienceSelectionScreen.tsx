...
const EXPERIENCES = [
  { id: "none", label: "No experience", icon: "placeholder-none" },
  { id: "little", label: "A little exposure", icon: "placeholder-little" },
  { id: "hands-on", label: "Hands-on", icon: "placeholder-handson" },
  { id: "pro", label: "Professional", icon: "placeholder-pro" },
];

export function ExperienceSelectionScreen({ onNext, onBack, careerTitle = "Software Engineer" }: { onNext?: () => void; onBack?: () => void; careerTitle?: string }) {
      ...
      <div className="flex-1 flex flex-col gap-8 px-4 pb-6">
        <div className="flex items-start gap-4 px-2">
          ...
          <h1 className="text-xl font-semibold leading-snug">
            Have you <span className="text-[#01A17F]">worked</span> with code before, on your way to <span className="text-[#01A17F]">{careerTitle}</span>?
          </h1>
        </div>
          ...
          {EXPERIENCES.map((exp) => {
            ...
            return (
              ...
              >
                ...
                <span className="font-medium text-sm text-center">{exp.label}</span>
              </button>
            );
          })}
        </div>
