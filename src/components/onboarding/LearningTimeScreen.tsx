...
const LEARNING_TIMES = [
  { id: "morning", label: "Morning", icon: "placeholder-morning" },
  { id: "afternoon", label: "Afternoon", icon: "placeholder-afternoon" },
  { id: "evening", label: "Evening", icon: "placeholder-evening" },
  { id: "night", label: "Night", icon: "placeholder-night" },
];

export function LearningTimeScreen({ onNext, onBack, careerTitle = "Software Engineering" }: { onNext?: () => void; onBack?: () => void; careerTitle?: string }) {
    ...
    <div className="flex flex-col min-h-screen bg-white dark:bg-[#0D1016] text-[#2C2C2C] dark:text-white font-sans max-w-md mx-auto relative">
      ...
      <header className="flex items-center justify-between px-4 pt-6 mb-6 shrink-0">
        ...
        <div className="flex flex-col items-center">
          <span className="text-xs font-bold text-[#01A17F] mb-1">07/07</span>
      ...
      <div className="flex-1 flex flex-col gap-8 px-4 pb-6">
        <div className="flex items-start gap-4 px-2">
          ...
          <h1 className="text-xl font-semibold leading-snug">
            What is the <span className="text-[#01A17F]">best time</span> in a day for you to learn <span className="text-[#01A17F]">{careerTitle}</span>?
          </h1>
        </div>
          ...
          {LEARNING_TIMES.map((time) => {
            ...
            return (
              ...
              >
                ...
                <span className="font-medium text-sm text-center">{time.label}</span>
              </button>
            );
          })}
        </div>
